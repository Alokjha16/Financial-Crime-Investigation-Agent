...
class CaseRepository:
    ...
    def get_evidence(self, case_id: str) -> List[Dict[str, Any]]:
        ...
        try:
            cursor = conn.cursor(dictionary=True)
            cursor.execute(
                "SELECT * FROM evidence WHERE case_id = %s ORDER BY created_at ASC",
                (case_id,),
            )
            rows = cursor.fetchall()
            for row in rows:
    ...
    def submit_agent_result(self, case_id: str, agent_result: Dict[str, Any]) -> None:
        """Persist agent result atomically: case update, evidence inserts, and audit logs."""
        conn = get_connection()
        try:
            cursor = conn.cursor()
            ...
            for trace_event in agent_result["investigation_trace"]:
                evidence_rows.append(
                    (
                        case_id,
                        "investigation_trace",
                        f"Investigation trace event: {trace_event.get('event_type')}",
                        "agent",
            ...
            evidence_rows.extend(
                [
                    (
                        ...
                        "agent",
                        json.dumps({"risk_breakdown": agent_result["risk_breakdown"]}),
                    ),
                    (
                        case_id,
                        "pattern_analysis",
                        ...
                        "agent",
                        json.dumps(agent_result["pattern_analysis"]),
                    ),
                    (
                        case_id,
                        "investigator_explanation",
                        "Investigator explanation",
                        agent_result["explanation_source"],
                        json.dumps(
                            {
